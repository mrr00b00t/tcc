...
# Generate dataset with overlap (not perfectly separable)
X, y = datasets.make_blobs(n_samples=100, centers=2, random_state=6, cluster_std=2)

# Train a linear SVM with soft margin (C<infinity)
svm_clf = SVC(kernel="linear", C=1)  # C is the regularization parameter, smaller values lead to softer margins
svm_clf.fit(X, y)
